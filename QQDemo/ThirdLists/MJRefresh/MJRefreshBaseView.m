...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 4.箭头图片
        NSString *imgPath=[[NSBundle mainBundle].resourcePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/arrow.png",MJRefreshBundleName]];
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:imgPath]];
...
{
    if (_state != MJRefreshStateRefreshing)        // 存储当前的contentInset
        _scrollViewInitInset = _scrollView.contentInset;
...
- (void)dealloc
{
    NSLog(@"finilize refresh view:%@",[self debugDescription]);
