...
- (void)calculate {
	if (!caption || [caption isEqualToString:@""]) {
		...
		if (!hasFixedSize) {
			if (p.accessoryPosition == ATMHudAccessoryPositionTop || p.accessoryPosition == ATMHudAccessoryPositionBottom || p.accessoryPosition == ATMHudAccessoryPositionCustom) {
				if (progress > 0) {
		...
		switch (p.accessoryPosition) {
			case ATMHudAccessoryPositionTop: {
				...
				imageRect.origin.x = (targetBounds.size.width-self.image.size.width)*0.5;
			...
			case ATMHudAccessoryPositionRight: {
				...
				imageRect.origin.y = (targetBounds.size.height-self.image.size.height)*0.5;
				imageRect.size = image.size;
            ...
            case ATMHudAccessoryPositionCustom:
            {
                [self.backgroundLayer removeFromSuperlayer];
                activityRect = CGRectMake((targetBounds.size.width-activitySize.width)*0.5, marginY + 100, activitySize.width, activitySize.height);
				
				imageRect = CGRectZero;
				imageRect.origin.x = (targetBounds.size.width-self.image.size.width)*0.5;
				imageRect.origin.y = marginY + 100;
				imageRect.size = image.size;
				
				progressRect = CGRectMake((targetBounds.size.width-progressRect.size.width)*0.5, marginY + 100, progressRect.size.width, progressRect.size.height);
				
				captionRect.origin.x = (targetBounds.size.width-captionSize.width)*0.5;
				captionRect.origin.y = adjustment+marginY + 100;
                break;
            }
		}
	}
...
- (CGSize)calculateSizeForQueueItem:(ATMHudQueueItem *)item {
	...
	if (!item.caption || [item.caption isEqualToString:@""]) {
		...
		CGFloat marginY = 0;
		if (!hasFixedSize) {
			if (item.accessoryPosition == ATMHudAccessoryPositionTop || item.accessoryPosition == ATMHudAccessoryPositionBottom || item.accessoryPosition == ATMHudAccessoryPositionCustom) {
				if (item.image) {
...
- (void)applyWithMode:(ATMHudApplyMode)mode {
    ...
    if (superViewHeight < [[UIScreen mainScreen] bounds].size.height- 44.0 - 20.0) {
        superViewHeight = [[UIScreen mainScreen] bounds].size.height - 44.0*2 - 20.0;
    }
    switch (mode) {
		case ATMHudApplyModeShow: {
			if (CGPointEqualToPoint(p.center, CGPointZero)) {
				self.frame = CGRectMake((self.superview.bounds.size.width-targetBounds.size.width)*0.5, (superViewHeight-targetBounds.size.height)*0.5, targetBounds.size.width, targetBounds.size.height);
			} else {
